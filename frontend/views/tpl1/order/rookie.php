...
<header class="mui-bar mui-bar-nav own-main-background-color" style="position: static">
    <a href="<?= \yii\helpers\Url::to('shopping/index') ?>"><span
            class="back mui-icon mui-icon-left-nav mui-pull-left"></span></a>

    <h1 class="mui-title">新手挂单</h1>
</header>
<div class="mui-content" id="xinshou">
    <div class="messagebox"><p class="xuxianbox" style="line-height:.4rem;text-align:center">
            认证用户可进行<?= $numbers['min'] ?>-<?= $numbers['max'] ?><?= $money_name ?>议价交易！</p>
    </div>
    <div class="guoneibox" style="position:relative">
        <ul class="guoneinum" style="margin-top:.15rem">
            <li><i style="font-size:.24rem;font-style:normal; color:#777777;">当前价：</i><br/><span><?= $now_price ?></span> <i
                    style="font-size:.24rem;font-style:normal; color:#777777;">元RMB</i></li>
        </ul>
        ...
        <div class="mui-input-row mui-input-range field-contain">
            <div><input type="range" id="field-range" step="1" value="1" min="1" max="20" v-model="range"></div>
            <div style="margin-top:.3rem;text-align:center"><input type="number" id="field-range-input"
                                                                   v-model.number="range" @blur="checkNumber(range)"
                                                                   @input="limitbig(range)" placeholder="请输入1的倍数">
            </div>
        </div>
        <div class="guoneitotal"><p>买入<span v-text="range"></span><?= $money_name; ?>，出价<span
            ...
            <div class="xuzhi" style="margin-top:.15rem">
                <!--<a href="<?= \yii\helpers\Url::to('shopping/xuzhi') ?>"
                                                            data-title="新手挂单交易须知"><span
                        class="iconfont">&#xe67f;</span> <span>新手挂单交易须知</span></a>--></div>
        </div>
        <ul class="tapbox2">
            <li class="my-nav tapactive" data-index="show3"><span>买家看板<i></i></span></li>
            <li class="my-nav" data-index="show4"><span>交易信箱<i></i></span></li>
        </ul>
        <ul class="showbox2" style="margin-bottom: 2rem">
            <li id="show3" class="show showactive">
                <div class="mui-scroll-wrapper scw" id="scroll3">
                    <div class="mui-scroll ms">
                        <ul class="messagebox" id="allOrder" href="#topPopover">
                            <!--<li>
                                <div class="code">
                                    <div class="codeleft">
                                        <div class="circle"></div>
                                        <p v-text="">单号：666777888999</p></div>
                                    <div class="time">06月12月 15时30分</div>
                                </div>
                                <div class="info">13212123123（ID：123456）挂单10VRC，单价2.00元。</div>
                                <div class="btnbox">
                                    <button class="heta" type="button">卖给ta
                                    </button>
                                </div>
                            </li>-->
...
<script src="/tpl/js/sub/xinshou.js" charset="utf-8"></script>
...
<script>
    ...
    $(function () {
        ...
        $("#guabuy").click(function () {
            ...
            var $this = $(this);
            $.post(url, {price: price, number: number, type: type, '_csrf-frontend': token}, function (response) {
                if (response.code == 0) {
                    $this.html('挂买单');
    ...
    var page = 1;
    var pagesize = 20;
    var url = "<?= \yii\helpers\Url::to(['order/list','type'=>$type]) ?>";
    mui.init({
        pullRefresh: {
