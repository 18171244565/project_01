var pageNo1 = 1, pageNo2 = 1, template = null, subWebview = null, home = null, yztime = 60, vue1 = new Vue({
	...
	created: function () {
		// getmarketdata("GetMyHangTran", "get", {
	...
	computed: {
		totalprice: function () {
			return (this.range * this.total).toFixed(2)
		}
	},
	methods: {
		limitsmall: function () {
		...
		// 						--yztime < 0 && (clearInterval(u), yztime = 60)
		// 					}, 1e3);
		...
		// 	})), template.show("slide-in-right", 150); else {
		// 		var i = ["确定", "取消"];
		// 		mui.prompt("请严格按照平台公布卖家资料进行打款，请勿相信中介！", "交易密码", "请输入交易密码", i, function (t) {
		...
		// 	e < 10 ? this.range = 10 : e > 200 && (this.range = 200)
		// }, limitbig: function (e) {
		// 	e > 10 && e % 10 != 0 ? (mui.toast("请输入10的倍数"), this.range = e - e % 10) : e > 200 && (this.range = 200)
		// }, buythis: function () {
		// 	return this.range < 10 ? (this.range = 10, void mui.toast("最小为10")) : this.range > 200 ? (this.range = 200, void mui.toast("最大为200")) : void mui.toast("交易成功")
		// }, getnewpage: function (e, t) {
		// 	0 == e ? getmarketdata("GetMyHangTran", "get", {
		// 		userid: localStorage.getItem("userid"),
		// 		pageno: t,
		// 		type: e
		// 	}, !0, function (e) {
	...
	mounted: function () {
		mui.init({swipeBack: !1}), mui.plusReady(function () {
			template = plus.webview.getWebviewById("template_second"), subWebview = plus.webview.getWebviewById("sub_template_second"), home = plus.webview.getWebviewById("barItem/mine.html"), new Clipboard(".copy"), mui(".mui-scroll-wrapper").scroll({
				...
				if (vue1.total < .1) return vue1.total = .1, void mui.toast("最小为0.1");
				if (vue1.range < 10) return vue1.range = 10, void mui.toast("最小为10");
				if (vue1.range > 200) return vue1.range = 200, void mui.toast("最大为200");
				mui.confirm("确认挂单吗？", "确认挂单", ["确认", "取消"], function (e) {
					0 == e.index && getmarketdata("StartHangTransfer", "get", {
						userid: localStorage.getItem("userid"),
						amount: vue1.range,
						price: vue1.total
					}, !0, function (e) {
				...
				"show4" == e && (vue1.list2.length > 0 || 0 == vue1.list2.length && getmarketdata("GetMyHangTran", "get", {
					userid: localStorage.getItem("userid"),
					pageno: 1,
					type: "1"
				}, !0, function (e) {
