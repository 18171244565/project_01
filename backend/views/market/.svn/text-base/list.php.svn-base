...
<div class="page-content">
    <form class="form-horizontal" method="post" action="<?php echo \yii\helpers\Url::to(['market/list']) ?>"
          role="form">
        ...
        <div style="margin-bottom: 15px;height: 30px">
            ...
            <div class="col-lg-2 col-sm-4">
                <select class="form-control" name="status">
                    ...
                    <option <?php if (\Yii::$app->request->post('status') == 4): ?>selected<?php endif; ?> value="4">
                        交易完成
                    </option>
                    <option <?php if (\Yii::$app->request->post('status') == 5): ?>selected<?php endif; ?> value="5">
                        交易取消
                    </option>
                </select>
            </div>
            <div class="col-lg-2 col-sm-4">
                <select class="form-control" name="type">
                    ...
                    <option <?php if (\Yii::$app->request->post('type') == 1): ?>selected<?php endif; ?> value="1">
                        新手挂单
                    </option>
                    <option <?php if (\Yii::$app->request->post('type') == 2): ?>selected<?php endif; ?> value="2">
                        进阶挂单
                    </option>
                    <option <?php if (\Yii::$app->request->post('type') == 3): ?>selected<?php endif; ?> value="3">
                        高手挂单
                    </option>
                </select>
            </div>
        ...
        <table id="sample-table-2" class="table table-striped table-bordered table-hover">
            ...
            <?php foreach ($list as $order): ?>
                <tr>
                    ...
                    <td class="center">
                        ID：<?= $order->buy_member_id ?><br>
                        mobile：<?= $order->getBuyMemberName() ?>
                    </td>
                    <td class="center">
                        数量：<?= $order->number ?><br>
                        手续费：<?= $order->change_money ?>
                    </td>
                    <td class="center">
                        <?= $order->price ?>
                    </td>
                    <td class="center">
                        <?= $order->getTypeText($order->type) ?>
                    </td>
                    <td class="center">
                        <?= $order->getStatusText() ?>
                    </td>
                    ...
                    <td class="center">
                        <a href="<?= \yii\helpers\Url::to(['market/detail', 'id' => $order->id]) ?>">查看详细</a>
...
<script>
    var url = "<?= \yii\helpers\Url::to(['market/cancel']) ?>";
